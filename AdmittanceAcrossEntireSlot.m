...
slot_2_length_x_f = (slot_2_x_dist)*lambda; slot_2_width_y_f = (dy)*lambda; 
...
slot_1_length_x_f = (slot_1_x_dist)*lambda; slot_1_width_y_f = (dy)*lambda;
...
for y = 2*dy:dy:y_sep
    ...
    for x = dx:dx:slot_1_length_x_f
        ...
        for x_prime = dx:dx:slot_2_length_x_f
            y_prime = dy;         
            H_field_1(index1,1) = -(exp(-k*((x - x_prime)^2 + (y - y_prime)^2)^(1/2)*1i)*(k^2*y^4 + k^2*y_prime^4 - 4*x*x_prime + 2*y*y_prime - k*((x - x_prime)^2 + (y - y_prime)^2)^(3/2)*1i + 2*x^2 + 2*x_prime^2 - y^2 - y_prime^2 - 4*k^2*y*y_prime^3 - 4*k^2*y^3*y_prime + k*x^2*((x - x_prime)^2 + (y - y_prime)^2)^(1/2)*3i + k*x_prime^2*((x - x_prime)^2 + (y - y_prime)^2)^(1/2)*3i + k^2*x^2*y^2 + k^2*x^2*y_prime^2 + k^2*x_prime^2*y^2 + k^2*x_prime^2*y_prime^2 + 6*k^2*y^2*y_prime^2 - k*x*x_prime*((x - x_prime)^2 + (y - y_prime)^2)^(1/2)*6i - 2*k^2*x*x_prime*y^2 - 2*k^2*x*x_prime*y_prime^2 - 2*k^2*x^2*y*y_prime - 2*k^2*x_prime^2*y*y_prime + 4*k^2*x*x_prime*y*y_prime))/(4*pi*((x - x_prime)^2 + (y - y_prime)^2)^(5/2));
            index1 = index1 + 1;
        end
