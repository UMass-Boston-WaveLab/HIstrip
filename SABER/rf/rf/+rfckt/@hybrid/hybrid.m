classdef (CaseInsensitiveProperties,TruncatedProperties) ...
        hybrid < rfckt.network
%rfckt.hybrid class
%   rfckt.hybrid extends rfckt.network.
%
%    rfckt.hybrid properties:
...
%
%    rfckt.hybrid methods:
%       checkproperty - Check the properties of the object.
%       nwa - Calculate the network parameters.


    methods  % constructor block
        function h = hybrid(varargin)
        %HYBRID Constructor.
        %   H = RFCKT.HYBRID('CKTS', {CKT1, CKT2, ...}) returns a hybrid
        %   network object, H, which contains a cell of 2-port RFCKT
        %   objects: CKT1, CKT2, .... The default CKTS is empty. 
        %
        %   A hybrid RFCKT object has the following properties. All the
        ...
        %   'help rfckt.hybrid/<METHOD>', where METHOD is one of the
        %   methods listed above. For instance, 'help rfckt.hybrid/plot'.
        %
        %   Example:
        %
        %      % Construct a hybrid RFCKT object
        %        h = rfckt.hybrid('Ckts', {rfckt.txline('LineLength',   ...
        ...
        %UDD % h = rfckt.hybrid;
        set(h, 'Name', 'Hybrid Connected Network');
