function scd_params = s2scd(s_params, rfflag)
%S2SCD Convert 2N-port S-parameters to cross-mode N-port S-parameters Scd
%   SCD_PARAMS = S2SCD(S_PARAMS, RFFLAG) coverts single-ended 2N-port
%   S-parameters to cross-mode N-port S-parameters Scd. 
%
%   S_PARAMS is a complex 2Nx2NxM array of M 2N-port single-ended
%   S-parameters. The output SCD_PARAMS is a complex NxNxM array of M
%   N-port cross-mode S-parameters.
%
%   S2SCD uses one of 3 definitions of port indices for 2N-port
...
%      
%   See also S2SDD, S2SCC, S2SDC, SMM2S, S2SMM
...
[~, ~, scd_params] = s2smm(s_params, rfflag);
