...
for y = 2*dy:dy:y_sep+dy  %Seperation distance values. 2dy used b/c 1dy causes weird graphing issues. 
    ...
    Admittance_WRT_distance(1,A_index) = sum(sum((1)*Temp_Matrix*(1/(1i*omega*u_0*V_1*V_2)))); %Magnetic current from slot 1 would be dotted here as well, thus providing another negative sign
    A_index = A_index + 1;
end
