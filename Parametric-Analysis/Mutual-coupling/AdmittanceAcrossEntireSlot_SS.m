...
for y = 2*dy:dy:y_sep+dy  %Seperation distance values. 2dy used b/c 1dy causes weird graphing issues. 
    ...
    for x = dx:dx:slot_1_length_x_f        %Non-Primed values
        ...
        for x_prime = dx:dx:slot_2_length_x_f          %Primed space
            y_prime = dy;         
            H_field_1(1,index1) = 3*1j*k*((x-x_prime)^2)*exp(-1j*k*sqrt((x-x_prime)^2 + (y - y_prime)^2))/(((x-x_prime)^2 + (y - y_prime)^2)^2)...
                                 + 3*((x-x_prime)^2)*exp(-1j*k*sqrt((x - x_prime)^2 + (y - y_prime)^2))/(((x-x_prime)^2 + (y - y_prime)^2)^(5/2))...
                                 -1j*k*exp(-1j*k*sqrt((x-x_prime)^2 + (y - y_prime)^2))/((x-x_prime)^2 + (y - y_prime)^2)...
                                 +((1j)^2)*(k^2)*((x-x_prime)^2)*exp(-1j*k*sqrt((x-x_prime)^2 + (y - y_prime)^2))/(((x-x_prime)^2 + (y - y_prime)^2)^(3/2))...
                                 -exp(-1j*k*sqrt((x-x_prime)^2 + (y - y_prime)^2))/(((x-x_prime)^2 + (y - y_prime)^2)^(3/2))...
                                 - (k^2)*exp(-1j*k*sqrt((x-x_prime)^2 + (y - y_prime)^2))/(((x-x_prime)^2 + (y - y_prime)^2)^(1/2));            
            
            %-(exp(-k*((x - x_prime)^2 + (y - y_prime)^2)^(1/2)*1i)*(k^2*y^4 + k^2*y_prime^4 - 4*x*x_prime + 2*y*y_prime - k*((x - x_prime)^2 + (y - y_prime)^2)^(3/2)*1i + 2*x^2 + 2*x_prime^2 - y^2 - y_prime^2 - 4*k^2*y*y_prime^3 - 4*k^2*y^3*y_prime + k*x^2*((x - x_prime)^2 + (y - y_prime)^2)^(1/2)*3i + k*x_prime^2*((x - x_prime)^2 + (y - y_prime)^2)^(1/2)*3i + k^2*x^2*y^2 + k^2*x^2*y_prime^2 + k^2*x_prime^2*y^2 + k^2*x_prime^2*y_prime^2 + 6*k^2*y^2*y_prime^2 - k*x*x_prime*((x - x_prime)^2 + (y - y_prime)^2)^(1/2)*6i - 2*k^2*x*x_prime*y^2 - 2*k^2*x*x_prime*y_prime^2 - 2*k^2*x^2*y*y_prime - 2*k^2*x_prime^2*y*y_prime + 4*k^2*x*x_prime*y*y_prime))/(4*pi*((x - x_prime)^2 + (y - y_prime)^2)^(5/2));
            % (exp(-k*((x - x_prime)^2 + (y - y_prime)^2)^(1/2)*1i)*(k^2*y^4 + k^2*y_prime^4 - 4*x*x_prime + 2*y*y_prime - k*((x - x_prime)^2 + (y - y_prime)^2)^(3/2)*1i + 2*x^2 + 2*x_prime^2 - y^2 - y_prime^2 - 4*k^2*y*y_prime^3 - 4*k^2*y^3*y_prime + k*x^2*((x - x_prime)^2 + (y - y_prime)^2)^(1/2)*3i + k*x_prime^2*((x - x_prime)^2 + (y - y_prime)^2)^(1/2)*3i + k^2*x^2*y^2 + k^2*x^2*y_prime^2 + k^2*x_prime^2*y^2 + k^2*x_prime^2*y_prime^2 + 6*k^2*y^2*y_prime^2 - k*x*x_prime*((x - x_prime)^2 + (y - y_prime)^2)^(1/2)*6i - 2*k^2*x*x_prime*y^2 - 2*k^2*x*x_prime*y_prime^2 - 2*k^2*x^2*y*y_prime - 2*k^2*x_prime^2*y*y_prime + 4*k^2*x*x_prime*y*y_prime))/((x - x_prime)^2 + (y - y_prime)^2)^(5/2)


            index1 = index1 + 1;
        end
        Temp_Matrix(1, T_index) = sum(sum(H_field_1))*(dy_prime*dx_prime);
        T_index = T_index + 1;
    end
    Admittance_WRT_distance(1,A_index) = sum(sum(Temp_Matrix))*(dx_prime*dy_prime)*(1/(1i*omega*u_0*V_1*V_2*4*pi)); %Magnetic current from slot 1 would be dotted here as well, thus providing another negative sign
    A_index = A_index + 1;
end
...
% assignin('base', 'Admittance', Admittance_WRT_distance);
